...
@section('breadcrumbs')
    ...
    @elseif ($filter->has('brand'))
        {{ Breadcrumbs::render('products-brand', $filter['brand']) }}
    @else
                    ...
                    <div id="product-filter" class="font-sans bg-white py-4 px-3 min-h-screen shadow-sm closed">
                        ...
                        <div class="filter-section">
                            ...
                            <form id="price-filter" action="" class="price-filter">
                                <input type="number" name="min_price" class="form-input rounded-none w-full py-1 my-2"
                                    placeholder="min" value="{{ request()->get('min_price') }}">
                                <input type="number" name="max_price" class="form-input rounded-none w-full py-1 mt-2"
                                    placeholder="max" value="{{ request()->get('max_price') }}">
                                <button
                                    class="block w-full bg-white border border-dark text-dark hover:bg-dark hover:text-white hover:shadow py-2 px-3 text-sm font-semibold mt-3">
                    ...
                    <div class="block bg-white text-sm font-light py-2 px-4 -mx-4 md:mx-0">
                        <div class="flex items-center">
                            ...
                            <div x-data="" class="md:hidden">
                                <button id="filter-overlay" x-on:click="closeFilterPanel()"
                                    class="hidden fixed top-0 left-0 w-full h-full bg-black opacity-25 cursor-default"></button>
                                <button x-on:click="openFilterPanel()"
                                    class="flex items-center space-x-2 p-1 px-2 hover:bg-gray-400 rounded" type="button">
                            ...
                            <div class="ml-auto">
                                <div x-data="{ open: false }" class="relative" @click.away="open = false">
                                    <span class="hidden md:inline-block text-gray-700 font-semibold mr-1">Sort
                                        By:</span>
                                    <button @click="open = !open"
                                        class="inline-flex py-1 px-3 border border-gray-500 hover:shadow-sm">
                                        <span>{{ request()->get('sort') == 'price-htl' ? 'High to Low' : (request()->get('sort') == 'price-lth' ? 'Low to High' : 'Best Match') }}</span>
                                        <span class="text-gray-600">
                                            <svg class="inline w-4 h-4 ml-1 transition-transform duration-200 transform"
                                                :class="{ 'rotate-180': open, 'rotate-0': !open }" fill="currentColor"
                                                viewBox="0 0 20 20">
                                    ...
                                    <div x-show="open"
                                        class="absolute right-0 bg-white border border-gray-400 p-2 z-10 text-xs" x-cloak>
                                        <a href="{{ route('frontend.products.index', Arr::except(request()->query(), ['sort']) + ['sort' => 'price-d']) }}"
                                            class="block py-2 px-4 rounded hover:bg-primary hover:text-white cursor-pointer">
                                            Best Match</a>
                                        <a href="{{ route('frontend.products.index', Arr::except(request()->query(), ['sort']) + ['sort' => 'price-lth']) }}"
                                            class="block py-2 px-4 rounded hover:bg-primary hover:text-white cursor-pointer">
                                            Price Low to High</a>
                                        <a href="{{ route('frontend.products.index', Arr::except(request()->query(), ['sort']) + ['sort' => 'price-htl']) }}"
                                            class="block py-2 px-4 rounded hover:bg-primary hover:text-white cursor-pointer">
                                            Price High to Low</a>
                                    </div>
                                </div>
...
@push('scripts')
    <script>
        $('#price-filter').submit(function(event) {
            ...
            let fullUrl =
                "{{ url()->current() . '?' . http_build_query(request()->except(['min_price', 'max_price'])) }}";
