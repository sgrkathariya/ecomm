<form action="{{ $product->exists ? route('products.update', $product) : route('products.store') }}" method="post" class="form" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-md-3">
            ...
            <x-box>
                <div class="form-group">
                    <label for="brand_id" class="required">Product Brand</label>
                    <select name="brand_id" class="custom-select rounded-0 {{ invalid_class('brand_id') }}">
                        <option value="">Select Brand</option>
                        @foreach ($brands as $brand)
                        <option value="{{ $brand->id }}" @if(old('brand_id', $product->brand_id) == $brand->id) selected @endif>{{ $brand->name }}</option>
                        @endforeach
                    </select>
                    <x-invalid-feedback field="brand_id"></x-invalid-feedback>
                </div>
            </x-box>
